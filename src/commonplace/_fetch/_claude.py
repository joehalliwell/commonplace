...
class ClaudeFetcher:
    ...
    def _last_import_time(self, repo: Commonplace) -> str | None:
        """ISO timestamp of the most recent commit touching this source's chats,
        or None if there's no such history yet. Beware: this is the commit time,
        which is always ≥ max(updated_at) of the last-imported convos, so a
        conversation edited on claude.ai *during* a fetch may be missed until
        it changes again."""
